...
export default function Home({ setCurrentPage }: HomeProps) {
  return (
    <section
      id="hero"
      className="grid grid-cols-1 lg:grid-cols-[1.2fr_1fr] gap-10 lg:gap-16 items-center py-10"
    >
      <div className="flex flex-col gap-6 animate-fade-up [animation-delay:200ms]">
        <div className="font-mono text-xs text-[#ff4d00] tracking-wider">
          // SS26 NEW DROP // RAW URBAN WEAR
        </div>
        ...
        <div className="flex gap-4 mt-3">
          <button
            ...
            className="bg-[#ff4d00] text-white cursor-pointer no-underline font-sans font-bold text-sm px-7 py-3.5 rounded-sm inline-flex items-center gap-2 hover:bg-[#e04400] hover:-translate-y-0.5 transition-all duration-200"
          >
            GET THE DROP <span>→</span>
          </button>
          <button
            onClick={() => setCurrentPage("ARCHIVE")}
            className="border border-[#d3d8e2] dark:border-[#22252a] text-[#0f1013] dark:text-[#e2e4e9] cursor-pointer no-underline font-sans font-bold text-sm px-7 py-3.5 rounded-sm inline-flex items-center hover:bg-white dark:hover:bg-[#1d2026] hover:border-[#5a6270] dark:hover:border-[#8b919d] transition-all duration-200"
          >
            EXPLORE ARCHIVE
          </button>
        </div>
      ...
      <div className="flex flex-col gap-4 animate-zoom-in [animation-delay:400ms]">
        <div className="relative bg-white dark:bg-[#14161b] border border-[#d3d8e2] dark:border-[#22252a] p-6 flex justify-center items-center overflow-hidden hover:border-[#5a6270] dark:hover:border-[#8b919d] group transition-all duration-300">
          <div className="absolute top-0 left-0 w-full h-full pointer-events-none">
            <span className="absolute top-3 left-3.5 font-mono text-sm font-light text-[#d3d8e2] dark:text-[#22252a]">
              +
            </span>
            <span className="absolute top-3 right-3.5 font-mono text-sm font-light text-[#d3d8e2] dark:text-[#22252a]">
              +
            </span>
            <span className="absolute bottom-3 left-3.5 font-mono text-sm font-light text-[#d3d8e2] dark:text-[#22252a]">
              +
            </span>
            <span className="absolute bottom-3 right-3.5 font-mono text-sm font-light text-[#d3d8e2] dark:text-[#22252a]">
              +
            </span>
          </div>
          <img
